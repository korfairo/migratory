...
Command creates migrations table if not exists.`,
	Example: `redo -c /etc/config.yml
redo -d postgresql://role:password@127.0.0.1:5432/database --dir example/migrations/
redo -d postgresql://role:password@127.0.0.1:5432/database --dir migrations/ -t my_migrations_table`,
	Run: func(cmd *cobra.Command, args []string) {
		if err := rollback(config.Directory, config.Schema, config.Table, true); err != nil {
